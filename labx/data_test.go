...
func TestLoadAllExtraData(t *testing.T) {
	testCases := []struct {
		name     string
		fsys     fstest.MapFS
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			result, err := loadAllExtraData(testCase.fsys, []fs.FS{})
...
func TestLoadAllExtraDataSpecialCases(t *testing.T) {
	t.Run("filename parsing with various patterns", func(t *testing.T) {
		...
		result, err := loadAllExtraData(fsys, []fs.FS{})
		require.NoError(t, err)
...
func TestLoadAllExtraDataPerformance(t *testing.T) {
...
func TestLoadAllExtraDataFromDirs(t *testing.T) {
	testCases := []struct {
		name         string
		rootFS       fstest.MapFS
		externalDirs []fs.FS
		...
		},
		{
			...
			rootFS: fstest.MapFS{
				"data/config.json": &fstest.MapFile{
					Data: []byte(`{"source": "root", "priority": "high"}`),
				},
			},
			externalDirs: []fs.FS{
				fstest.MapFS{
					"config.json": &fstest.MapFile{
						Data: []byte(`{"source": "external", "value": 100}`),
					},
				},
			},
			expected: map[string]any{
		...
		},
		{
			...
			rootFS: fstest.MapFS{
				"data/config.json": &fstest.MapFile{
					Data: []byte(`{"env": "production", "debug": false}`),
				},
			},
			externalDirs: []fs.FS{
				fstest.MapFS{
					"config.json": &fstest.MapFile{
						Data: []byte(`{"env": "development", "debug": true, "extra": "value"}`),
					},
				},
			},
			expected: map[string]any{
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			result, err := loadAllExtraData(testCase.rootFS, testCase.externalDirs)
